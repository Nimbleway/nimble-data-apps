...
Built with [Nimble Web Search Agents](https://nimbleway.com) via `langchain-nimble`. Change the query and it becomes a funding monitor, competitor tracker, acquisition watcher, pricing monitor, or industry-alert bot.
...
```cron
0 */6 * * * cd /path/to/nimble-data-apps/deal-monitor && /path/to/venv/bin/python agent.py >> monitor.log 2>&1
```
